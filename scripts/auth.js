...
export async function linkToOwnerByEmail(ownerEmail) {
  const user = auth.currentUser;
  if (!user) throw new Error("לא מחובר/ת");
  // חיפוש ownerUid לפי אימייל (אמור להיות ייחודי)
  ...
  const linkRef = doc(db, "links", ownerUid, "members", user.uid);
...
// שליפת חשבונות מקושרים למשתמש הנוכחי (collectionGroup 'members')
export async function getLinkedOwnersForCurrentUser() {
  ...
  // ownerUid מהנתיב: links/{ownerUid}/members/{memberUid}
